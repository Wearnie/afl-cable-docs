...
export default function UploadPage() {
  ...
  const isReady = djNumber.length >= 4 && productCode.length === 13 && file
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await saveCert({ djNumber, productCode, file })
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-afl-border rounded-xl font-mono text-lg tracking-[0.15em] uppercase focus:outline-none focus:ring-2 focus:ring-afl-cyan focus:border-transparent transition-shadow"
              />
              <span className={`text-xs font-medium mt-1 block ${productCode.length === 13 ? 'text-emerald-600' : 'text-afl-muted'}`}>
                {productCode.length}/13
              </span>
            </div>
          </div>
                ...
                <div key={cert.djNumber} className="flex items-center gap-3 px-3 py-2.5 rounded-lg bg-gray-50 border border-gray-100">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono text-xs text-afl-muted">{cert.productCode}</span>
                    </div>
                    <p className="text-[11px] text-afl-muted truncate">{cert.fileName}</p>
                  </div>
                  <Link
                    to={`/${cert.productCode}?dj=${cert.djNumber}`}
